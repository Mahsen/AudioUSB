...
void wait_delay(int count)
{
	...
	while(count--)
        for(j_delay = 0; j_delay < 1141; j_delay++ );
...
uint16_t LCD_Read (void) 
{
	...
	value = LPC_GPIO2->FIOPIN;                 /* Read D8..D15 */
	LCD_EN(1);	                               /* Enable 1B->1A */
	wait_delay(30);							   /* delay some times */
	value = (value << 8) | LPC_GPIO2->FIOPIN;  /* Read D0..D7 */
	LCD_DIR(1);
	return  value;
}
...
void LCD_Initializtion(void)
{
	...
	else if( DeviceCode == 0x8989 )
	{
	    ...
	    LCD_WriteReg(0x000F,0x0000);    delay_ms(50);   /* ?¨?è???¼µ??· */
	...
	else if( DeviceCode == 0x5408 )
	{
		...
		LCD_WriteReg(0x0003,0x1030);	  /* Entry Mode?è?? */
		...
		LCD_WriteReg(0x0050,0x0000);	  /* ?è??²?×÷´°??µ?X?????¼?? */
		LCD_WriteReg(0x0051,0x00EF);	  /* ?è??²?×÷´°??µ?X??½????? */
		LCD_WriteReg(0x0052,0x0000);	  /* ?è??²?×÷´°??µ?Y?????¼?? */
		LCD_WriteReg(0x0053,0x013F);	  /* ?è??²?×÷´°??µ?Y??½????? */
...
void PutChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    ...
    for( i=0; i<16; i++ )
    {
        ...
        for( j=0; j<8; j++ )
        {
            ...
            {
                LCD_SetPoint( Xpos + j, Ypos + i, charColor );  /* ×?·û???« */
            }
            else
            {
...
void LCD_WritePic(uint16_t Xpos, uint16_t Ypos,int H_Size, int V_Size, int sinc_counter_y, unsigned short *pointer)
 {
     int x1 = Xpos + (H_Size), y1 = Ypos + (V_Size),counter_x,counter_y,counter,sinc_number;
     
     //if(Xpos < 0) Xpos = 0;
     //if(Ypos < 0) Ypos = 0;
     
     for(sinc_number=0;sinc_number<sinc_counter_y;sinc_number++)
     {
     
         counter=(x1*sinc_number);
         
         for(counter_y=sinc_number;counter_y < y1;counter_y+=sinc_counter_y)
           
         {
                for(counter_x = Xpos;counter_x < x1;counter_x++,counter++)
                        LCD_SetPoint(counter_x,counter_y,pointer[counter]);
                        
    
                counter+=(x1*(sinc_counter_y-1));
